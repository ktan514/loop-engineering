...
環境変数名を指定するconfig fieldは、その名前をcredential取得先のAuthorityとする。Python側は`.env`を読まず、configから得た名前をkeyとして現在processのenvironmentを参照する。OpenAI reviewerでは環境変数名を`OPENAI_API_KEY`へ固定し、別名は設定エラーとする。
...
- OpenAI reviewerのcredential環境変数名は`OPENAI_API_KEY`へ固定する
- `.env`をcommitしない
