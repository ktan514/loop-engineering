...
def test_write_gate_rejects_wrong_project_stale_head_stale_field_and_trunk() -> None:
    cfg = config()
    wrong_project = WriteIntent("i", "project", "6", "edit", (), (), "epoch")
    stale = WriteIntent(
    ...
    trunk = WriteIntent(
        ...
        "content",
        (("branch_ref", "rebuild/v2-foundation"),),
        (("effect", "x"),),
        "epoch",
    )
    assert (
        validate(wrong_project, {}, config=cfg).conflict
        is ConflictKind.FORBIDDEN_PROJECT_IDENTITY
    )
    assert (
        validate(stale, {"head": "new"}, config=cfg).conflict
        is ConflictKind.STALE_WRITE_GATE
    )
    assert (
        validate(stale_field, {"field_id": "new"}, config=cfg).conflict
        is ConflictKind.STALE_WRITE_GATE
    )
    assert (
        validate(trunk, {}, config=cfg).conflict
...
def test_write_gate_requires_exact_precondition_and_effect_readback() -> None:
    ...
    mismatch = validate(intent, {"head": "h"}, {"status": "In progress"}, config=cfg)
    passed = validate(intent, {"head": "h"}, {"status": "Done"}, config=cfg)
    missing = validate(intent, {"head": "h"}, config=cfg)
...
def test_write_gate_rejects_noop_and_content_without_exact_identity() -> None:
    ...
    assert (
        validate(noop, {}, config=cfg).conflict is ConflictKind.NO_OP_MUTATION_FORBIDDEN
    )
    assert (
        validate(unknown_branch, {"branch_ref": "feature/x"}, config=cfg).conflict
