...
class FakeRunner:
    ...
    def run(
        ...
        if call[:3] == ("gh", "api", "graphql"):
            payload = {
                "data": {
                    "user": {"projectV2": {"viewerCanUpdate": self.project_write}}
                }
            }
            return CommandResult(True, json.dumps(payload))
        if call == ("gh", "api", "repos/ktan514/ai-liver-yura"):
            return CommandResult(
                True, json.dumps({"permissions": {"push": self.repository_write}})
            )
        return CommandResult(
            not any(call[: len(prefix)] == prefix for prefix in self.failed)
        )
...
def environment() -> dict[str, str]:
    return {
        "LOOP_TRUSTED_REVIEWER_SOCKET": "/private/tmp/yura-reviewer.sock",
...
def make_preflight(
    ...
    *,
    ...
    return EnvironmentCapabilityPreflight(
        config(),
        runner,
        env or environment(),
        reviewer_probe=reviewer or FakeReviewer(True),
        project_root=goal_root(tmp_path),
    )


def test_project_write_is_live_graphql_evidence_not_an_injected_boolean(
    tmp_path: Path,
) -> None:
    runner = FakeRunner(project_write=True)
    result = make_preflight(runner, tmp_path).run()
...
def test_repository_write_denial_is_fail_closed(tmp_path: Path) -> None:
    result = make_preflight(FakeRunner(repository_write=False), tmp_path).run()
...
def test_project_write_denial_blocks_without_project_mutation(tmp_path: Path) -> None:
    result = make_preflight(FakeRunner(project_write=False), tmp_path).run()

    assert result.status is PreflightStatus.BLOCKED
    assert "GITHUB_PROJECT_WRITE" in result.blocking_for_loop_bootstrap


def test_reviewer_requires_trusted_broker_and_never_receives_reviewer_key(
    tmp_path: Path,
) -> None:
    reviewer = FakeReviewer(False)
    env = environment() | {"OPENAI_API_KEY_REVIEWER": "reviewer-secret"}
    result = make_preflight(FakeRunner(), tmp_path, env=env, reviewer=reviewer).run()

    assert not result.capabilities["trusted_reviewer"]
    assert "TRUSTED_REVIEWER" in result.work_scoped_unavailable
...
def test_postgresql_separates_client_server_database_and_migration(tmp_path: Path) -> None:
    ...
    env = environment() | {
        "LOOP_DATABASE_URL": "postgresql://user:password@db.example:5432/loop"
    }
    runner = FakeRunner((("pg_isready",),))
    result = EnvironmentCapabilityPreflight(
        config(), runner, env, reviewer_probe=FakeReviewer(True), project_root=root
    ).run()
...
def test_goal_generation_mismatch_is_blocking(tmp_path: Path) -> None:
    ...
    result = make_preflight(FakeRunner(), tmp_path, env=env).run()
...
def test_postgresql_probe_preserves_only_path_and_pg_environment(tmp_path: Path) -> None:
    ...
    database_environment = next(
        probe_environment
        for call, probe_environment in zip(runner.calls, runner.environments, strict=True)
        if call == ("pg_isready",)
    )
...
def test_postgresql_malformed_port_fails_closed_and_remains_secret_safe(
    tmp_path: Path,
) -> None:
    env = environment() | {
...
def test_timeout_becomes_a_typed_diagnostic(tmp_path: Path) -> None:
    ...
    result = make_preflight(TimeoutRunner(), tmp_path).run()
