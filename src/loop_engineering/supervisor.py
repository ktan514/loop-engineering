...
class MissionSupervisor:
    """Makes decisions from supplied live observations; it has no provider transport."""
    ...
    def _decide_primary(self, epoch: ObservationEpoch) -> SupervisorDecision:
        ...
        eligible = tuple(
            work for work in epoch.works if not reconcile_work(epoch, work.issue_number)
        )
    ...
    def _packet(self, epoch: ObservationEpoch, work: WorkSnapshot, key: str) -> TaskPacket:
        ...
        return TaskPacket(
            ...
            ("reviewer transport", "operational database", "product runtime scheduler"),
            exact,
            ("live dependency evidence",),
            ("targeted tests", "Ruff", "strict Mypy", "full pytest", "exact-head CI"),
            (
                f"Project #{self.config.project_number} only",
                "no secrets",
                "no direct trunk write",
            ),
            lineage.identity.stable_id if lineage else None,
            "IMPLEMENT",
        )
